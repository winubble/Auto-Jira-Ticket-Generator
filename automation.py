...
def update_ticket(row, childEN):
    ...
    # update the linkedto
    jira.create_issue_link("Related", myissue, __outwardIssue, None)
...
for index, row in df_deploy.iterrows():
    ...
    if(row['Port'] == currPort and meetInitialPort == True):
        ...
        if(isinstance(row['Work Ticket'], str)):
            ...
            # this is wrong: row['Linked To'] = currEN

            # fill the linkedTo to row as well, becasue I pass the row to the method
            row['Linked To'] = currEN
            
            # update the jira ticket
            update_ticket(row, childEN)
            
        else:
            # Then it does not have a ticket
            # fill the linkedto cell(both row and df) with currEN and create a new ticket and write the URL to dataframe
